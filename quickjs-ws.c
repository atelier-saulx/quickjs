...
#include "quickjs-ws.h"
#include <curl/curl.h>
#include <libwebsockets.h>
#include "cutils.h"

JSModuleDef* js_init_module_ws(JSContext* ctx, const char* module_name) {
    JSModuleDef* m;
...
static JSValue create_websocket_obj(JSContext* ctx, struct lws* wsi) {
    ...
    if (JS_IsException(ws_obj)) return JS_EXCEPTION;
...
static JSValue minnet_ws_server(JSContext* ctx,
    ...
    if (JS_IsNumber(opt_port)) JS_ToInt32(ctx, &port, opt_port);

    if (JS_IsString(opt_host)) host = JS_ToCString(ctx, opt_host);
    else host = "localhost";
...
static const char* client_server_path = "/";
...
static JSValue minnet_ws_client(JSContext* ctx,
                                JSValueConst this_val,
                                int argc,
                                JSValueConst* argv) {
    struct lws_context_creation_info info;
    ...
    if (JS_IsString(opt_path)) {
        client_server_path = JS_ToCString(ctx, opt_path);
    }
...
static JSValue minnet_ws_close(JSContext* ctx,
                               JSValueConst this_val,
                               int argc,
                               JSValueConst* argv) {
    MinnetWebsocket* ws_obj;
...
static void minnet_ws_finalizer(JSRuntime* rt, JSValue val) {
    ...
    if (ws_obj) js_free_rt(rt, ws_obj);
}
...
static JSValue minnet_fetch(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
    ...
    if (JS_IsException(resObj)) return JS_EXCEPTION;
...
static JSValue minnet_response_json(JSContext* ctx,
                                    ...
                                    JSValueConst* argv) {
    MinnetResponse* res = JS_GetOpaque(this_val, minnet_response_class_id);
    if (res && res->buffer) return JS_ParseJSON(ctx, (char*)res->buffer, res->size, "<input>");
...
static JSValue minnet_response_text(JSContext* ctx,
                                    ...
                                    JSValueConst* argv) {
    MinnetResponse* res = JS_GetOpaque(this_val, minnet_response_class_id);
    if (res && res->buffer) return JS_NewStringLen(ctx, (char*)res->buffer, res->size);
